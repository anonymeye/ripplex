...
import { AppState, Todo, CounterState, TodoState, AsyncState, AppCoeffects } from './types'
...
export const asyncEventDefinitions = {
  ...
  [asyncEvents.fetchSuccess]: {
    interceptors: [
      after<AppState>((db: AppState, _effects: EffectMap) => {
  ...
  [asyncEvents.fetch]: {
    ...
    interceptors: [
      path<AppState>(['async']),
      debug<AppState>(),
      after<AppState>((_db: AppState, _effects: EffectMap) => {
